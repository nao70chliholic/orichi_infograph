...
**キャラクター画像はリポジトリに含めていない**（公開リポジトリのため）。
再クローンしたときは、使う画像を手元で `orochi_infograph/assets/` に置く。

| コミュニティ | ファイル名 |
|---|---|
| 開運オロチ | `plush_orochi.png`（追跡している） |
| CNPスタープロジェクト | `MAKAMIshinonome.jpg`（**追跡していない。手動で置く**） |

置き忘れると、画像は生成されるがイラストだけが描画されない（エラーにはならない）。
